...
# python convert.py
# 或者执行转换并打开Excel文件
# python convert.py open

# 判断是否传入参数open，如果有则打开Excel文件
if len(sys.argv) > 1 and sys.argv[1].lower() == 'open':
    ...
    if os.path.exists(excel_path):
        try:
            ...
            print(f"\033[33m文件 '{excel_path}' 可能已被打开，请关闭后重试。\033[0m")  # 黄色高亮
            exit(1)
    # 执行转换
    convert_to_excel(db_name)
    if os.path.exists(excel_path):
        os.startfile(excel_path)  # Windows下打开文件
